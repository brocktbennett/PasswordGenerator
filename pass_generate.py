...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if length < 1:
                slow_print("A password should have at least 1 character, c'mon! 🙃")
            else:
                break
        except ValueError:
            slow_print("That's not a number! Try again, genius! 🤓")
    ...
    purpose = input("What's this password for? Make it juicy! 🍉: ")

    # Generate the password
    password = generate_password(length, use_lower, use_upper, use_numbers)

    if password:
        slow_print(f"🌟 Your super-secure, absolutely amazing password is: {password} 🌟")

        # Save the password to a text file
        with open("passwords.txt", "a") as f:
            f.write(f"Juicy Details: {purpose}\n")
            f.write(f"Super-Secret Password: {password}\n")
            f.write("=" * 30 + "\n")

        slow_print("📝 Your password is now saved in a 'passwords.txt' file, as safe as a treasure chest! 🏴‍☠️")
